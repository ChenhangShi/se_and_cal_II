...
- AccountController.login
    ...
    - 前置条件  无
    - 后置条件  调用AccountService.login方法，成功则返回content为本用户的信息，失败则返回无content 
- AccountController.register
    - 语法     public ResponseVO registerAccount(@RequestBody UserVO userVO)
    - 前置条件  无
    - 后置条件  调用AccountService.registerAccount方法，成功则返回content为true，失败则返回无content     
- OrderController.addOrder
    - 语法     public ResponseVO addOrder(@RequestBody OrderVO orderVO)
    - 前置条件  无
    - 后置条件  调用OrderService.addOrder方法，成功则返回content为true，失败则返回无content         
...
- AccountService.login
    ...
    - 前置条件  无
    - 后置条件  调用AccountMapper.getAccountByName方法，取得用户信息，调用Methods.md5Encryption方法，如果加密后的密码等于数据库中的密码，返回本用户；如果用户不存在或密码不正确，返回null
- AccountService.registerAccount
    - 语法     public ResponseVO registerAccount(UserVO userVO)
    - 前置条件  无
    - 后置条件  调用Methods.md5Encryption方法，加密密码；调用AccountMapper.createNewAccount方法，成功则返回content为true的ResponseVO，失败则返回无content的ResponseVO  
- OrderService.addOrder
    - 语法     public ResponseVO addOrder(OrderVO orderVO)
    - 前置条件  无
    - 后置条件  调用HotelService.getRoomCurNum，如果剩余房间数目不足，失败；调用OrderMapper.addOrder方法；调用HotelService.updateRoomInfo方法，减少剩余房间数量。成功则返回content为true的ResponseVO，失败则返回无content的ResponseVO        
